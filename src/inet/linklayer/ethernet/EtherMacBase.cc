...
void EtherMacBase::initializeFlags()
{
    ...
    std::cout << "connected " << (physOutGate->getPathEndGate()->isConnected() ?  "true " : "false ") << (physInGate->getPathEndGate()->isConnected() ?  "true" : "false") << std::endl;

    if (!connected)
        std::cout << "MAC not connected to a network.\n" << std::flush;
